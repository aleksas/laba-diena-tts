...
extern "C" {
	...
	#include "../include/LithUSS.h"
	#include "../include/raw2wav.h"
}
	...
	for (int ii = 0; ii < 4; ii++)
	{
		...
		if (hr == 0)
		{
			...
			for (k = 0; k < pastrSk; k++)
			{
				if (pDataArray[k] != NULL)
				{
					if (pDataArray[k]->largebf != NULL)
						free(pDataArray[k]->largebf);
					if (pDataArray[k]->evar != NULL)
						free(pDataArray[k]->evar);
					free (pDataArray[k]);
					pDataArray[k] = NULL;
				}
			}
