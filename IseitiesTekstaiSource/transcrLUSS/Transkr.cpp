...
struct taisykles {char KKont[15]; char ES; char DKont1[15]; char DKont2[15];
                  unsigned char Kirt; unsigned char Minkst;
				  unsigned char SkDu; unsigned char SkPab;
				  char FonV[5]; char PoslR; char PoslT;} Taisykl[MAX_RULES];
/*
...
if(hr == NO_ERR)
	...
	for(j=0; j<rkiek; j++)
		{
		if(Taisykl[j].KKont[0] == '*') Taisykl[j].KKont[0] = 0;
		if(Taisykl[j].DKont1[0] == '*') Taisykl[j].DKont1[0] = 0;
		if(Taisykl[j].DKont2[0] == '*') Taisykl[j].DKont2[0] = 0;
		if(Taisykl[j].FonV[0] == '*') Taisykl[j].FonV[0] = 0;
		}
	initKircLUSS();
	}
...
eil[0]='_'; eil[ilg]=0; eil[ilg+1]='_'; eil[ilg+2]='_'; eil[ilg+3]=0;
// Taisykliu paieska
int k;
i=1;
j=0;
while(i<ilg)
{
if(Taisykl[j].ES!=eil[i]) j+=Taisykl[j].PoslT;
else
 {
 ...
 if(((Taisykl[j].KKont[0]!=0) ? (int)strchr(Taisykl[j].KKont, eil[i-1]):1)
   ...
   &&(((Taisykl[j].SkPab & SkPb[i] & 7)==(SkPb[i] & 7))||((Taisykl[j].SkPab & SkPb[i] & 7)==Taisykl[j].SkPab)
   ||((Taisykl[j].SkPab==8)&&(SkPb[i] & 8)))
   &&(((Taisykl[j].Minkst & 4)==0)||Trmp[i]))
   {
	...
	if(strlen(TrEil)<TrEilIlg-8)        //fv ilgis yra iki 4 raidziu ir \n+\n_
		...
		if(Taisykl[j].PoslR > 1) 
			...
			if(SkPb[i-2] & 8) {TrEil[ii]='+'; unitsRNextSep[fnr-1]='+';}
			else if(SkPb[i-2] & 2) {if(TrEil[ii]==' ') {TrEil[ii]='-'; unitsRNextSep[fnr-1]='-';}}
			if(eil[i-1]==0) i++;
			}
		if(Taisykl[j].FonV[0]!=0) 
			...
			if(SkPb[i-1] & 8) {strcat(TrEil, "+"); unitsRNextSep[fnr]='+';}
			else if((SkPb[i-1] & 2) && (Taisykl[j].PoslR>0))  {strcat(TrEil, "-"); unitsRNextSep[fnr]='-';}
			else {strcat(TrEil, " "); unitsRNextSep[fnr]=' ';}
			fnr++;
			}
		else
			{
			int ii=strlen(TrEil)-1;
			if(SkPb[i-1] & 8) {TrEil[ii]='+'; unitsRNextSep[fnr-1]='+';}
			else if(SkPb[i-1] & 2) {if(TrEil[ii]==' ') {TrEil[ii]='-'; unitsRNextSep[fnr-1]='-';}}
			}
		}
	else
		return -1;
	if(Taisykl[j].PoslR == 0) {j+=Taisykl[j].PoslT;} else {j=0;}
   }
 else j++;
 }
if(j>rkiek-1) {j=0; i++;}
